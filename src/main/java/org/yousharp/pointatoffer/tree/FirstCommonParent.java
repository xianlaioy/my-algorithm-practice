 ...
 *
 * 思路：
 *  分析一：如果这个一棵二叉搜索树，由于节点值之间的大小关系，遍历二叉树，找到这样一个节点，其节点值在两
 *      个已知节点值之间；
 *  分析二：如果二叉树中的每个节点都有指向其父节点的指针，则问题转换为求两个单链表的第一个公共节点；
 *  分析三：如果这只是一棵普通的二叉树，则有两种思路：
 *      思路一：分别求根节点到两个已知节点的路径，第一个公共父节点即为两条路径的最后相交节点。
 *      思路二：对于公共父节点，则两个已知节点必位于其左右子树中。此时又有两种思路，一种是直接遍历二叉树，
 *      判断节点是否在二叉树中，该思路中有的节点会被重复遍历多次；另一种思路是，先判断树中已知节点的个数，
 *      然后判断公共父节点。
