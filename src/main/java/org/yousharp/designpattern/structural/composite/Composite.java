 ...
 *  组合对象的不同而统一使用时，可以考虑使用组合模式；
 *
 * 解决的问题：一般来说，单个对象和组合对象的行为不同，用户的处理方式不同，因此
 *  在使用时需要先判断对象的类型，然后再使用；使用组合模式，单个对象与组合对象
 *  具有一致的接口，对用户来说是透明的；
 *
 * 案例：
 ...
 * 组合对象
 */
class MyDirectory implements FileInterface {
    private String name;
    private ArrayList<FileInterface> container;
    ...
    public void list() {
        ...
        for (FileInterface file: container) {
            file.list();
        }
    }
}
